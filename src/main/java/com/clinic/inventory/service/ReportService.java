...
public class ReportService {
        ...
        public List<ReportDtos.StockBalanceRow> stockBalance(LocalDate from,
                ...
                // Reconstruct stock as it was at the end of the reporting period, by adding
                // back
                // any transactions dated AFTER "to" that have already happened in real life.
                LocalDate farFuture = LocalDate.of(9999, 12, 31);
                var receivingAfter = receivingRepository
                                .findByStatusAndDateReceivedBetweenOrderByDateReceivedAsc(
                                                ReceivingStatus.APPROVED, to.plusDays(1), farFuture);
                var issuancesAfter = issuanceRepository
                                .findByDateIssuedBetweenOrderByDateIssuedAsc(to.plusDays(1), farFuture);
                var disposalsAfter = disposalRepository
                                .findByDisposalDateBetweenOrderByDisposalDateAsc(to.plusDays(1), farFuture);
                List<ReportDtos.StockBalanceRow> rows = new ArrayList<>();
                for (Item item : items) {
                        int liveCurrent = batchRepository
                                        .findByItemIdAndStatus(item.getId(), BatchStatus.ACTIVE)
                                        .stream().mapToInt(Batch::getOnHand).sum();
                        int deliveredAfter = receivingAfter.stream()
                                        .flatMap(r -> r.getLines().stream())
                                        .filter(l -> l.getItem().getId().equals(item.getId()))
                                        .mapToInt(ReceivingLine::getQuantity).sum();
                        int dispensedAfter = issuancesAfter.stream()
                                        .flatMap(i -> i.getLines().stream())
                                        .filter(l -> l.getItem().getId().equals(item.getId()))
                                        .mapToInt(IssuanceLine::getQuantity).sum();
                        int disposedAfter = disposalsAfter.stream()
                                        .filter(d -> d.getItem().getId().equals(item.getId()))
                                        .mapToInt(DisposalRecord::getQuantity).sum();
                        int current = liveCurrent - deliveredAfter + dispensedAfter
                                        + disposedAfter;
